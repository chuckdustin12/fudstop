import requests
import pandas as pd
payload={"pageSize":20,"pageIndex":0,"sortNewestToOldest":False,"searchText":None,"isSearchAll":True,"eventType":0}
headers={'cookie': 'FedAuth=; FedAuth1=; _hjSessionUser_1494715=eyJpZCI6ImQ0ZTI4MjhlLWY3YjUtNWNiZS1hMjRkLWUzMzY2YjJmZThiZCIsImNyZWF0ZWQiOjE3MzI3MDczMTIxNDcsImV4aXN0aW5nIjp0cnVlfQ==; __gsas=ID=aa879751e6250bbb:T=1744296499:RT=1744296499:S=ALNI_MaaJBLi6U39L2ATSZjEOSjKxTHiqA; __utma=211709392.586291915.1732748298.1747791979.1747838199.61; __utmc=211709392; __utmz=211709392.1747838199.61.52.utmcsr=google|utmccn=(organic)|utmcmd=organic|utmctr=(not%20provided); IdSvr.WsFedTracking=2ybS2yy5urXNrOlLZsMutEA8m20S6hhr3VrvINdZ5X_dhw5MyoWMWaX14BPkaYKrb8MV6SNi2b_BhJHRJXQn8c7sAIoadp7_SuE6omJpJ8FqVCUud8WK7TV-cqMSfxKMeoRreJM_JhQ29DOP_W_oUMqga041WITnaOYz9btAktD28gOrmVjeidpK44J9ET1JWllgJZSActvngfTRXmU3u4uOIXEzNm5tkcAAZQlsxP6ZPnx0xoAqZR44GkYhIQ0GgI_1lirYAvrQjzsskffqZlxLkT3sd1Gn5djxf1DXkJY; _hjSession_1494715=eyJpZCI6ImU0N2MxOGIzLWIyYjgtNGE5Yy1hMDcyLTk2MDVlNWI2YzIxYiIsImMiOjE3NDc5MjQ3MzM4ODIsInMiOjAsInIiOjAsInNiIjowLCJzciI6MCwic2UiOjAsImZzIjowfQ==; logged_in=true; idle_timer=2025-05-22T14:39:06.715Z'}

def fetch_all_case_events(case_id: str) -> pd.DataFrame:
    """Fetch and flatten all documents from event records for a given case ID."""
    url = f"https://research.txcourts.gov/CourtRecordsSearch/case/{case_id}/events"
    page_index = 0
    all_documents = []

    while True:
        payload = {
            "pageSize": 200,
            "pageIndex": page_index,
            "sortNewestToOldest": True,
            "searchText": None,
            "isSearchAll": True,
            "eventType": 0
        }
        r = requests.post(url, json=payload, headers=headers)
        r.raise_for_status()
        events = r.json().get("events", [])

        if not events:
            break

        for event in events:
            event_description = event.get("description")
            docs = event.get("documents", [])
            for doc in docs:
                # Add event-level metadata to each document
                doc["event_description"] = event_description
                doc["case_id"] = case_id
                all_documents.append(doc)

        page_index += 1

    df = pd.DataFrame(all_documents)

    # Optional: Rename columns for clarity
    df.rename(columns={
        "fileName": "name",
        "filingId": "filing_id",
        "filingCode": "filing_code",
        "filingCodeDescription": "filing_code_description",
        "filingDateUtc": "filing_date",
        "fileSize": "file_size",
        "documentCategoryCode": "document_category",
        "documentID": "document_id",
        "documentKey": "document_key"
    }, inplace=True)

    return df[[
        'case_id', 'event_description', 'document_category', 'document_id',
        'document_key', 'name', 'filing_id',
        'filing_code', 'filing_code_description', 'filing_date', 'file_size'
    ]]


case_dict = {
            "322-744263-23": "b04183e55b355b628fa3d90671422f9b", 
             "322-744538-23": "64cb30ca45885ab48035fbdc47973d6c",
             "233-765358-25": "b91f14f71df440179c53cb9e609b0a95",
             "02-24-00149-CV": "9628dfb9bf005983b1f101a8687f29ab", 
             "24-0395": "a961905f5a2e57d29f2d5436932fdbfe", 
             "02-25-00164-CV": "730bf651ae6945489c1928a6ddbe0d3d", 
             "02-25-00171-CV": "3a939a9942f34058bfc36a1d75c51e83", 
             "02-25-00166-CV": "85636dceaa4d45a4945830ada51632fe", 
             "25-0361": "aac67c62ff5e4f21af99abb9041454ec", 
             "25-0367": "6055079876534410bc13921fce4c2677", 
             "25-0378": "baa3ae6d61c247cb96111cb4182a3893", 
             "25-0426": "8a8aaaeba7744a25b343bf83a142923f"}

case_ids = ["b04183e55b355b628fa3d90671422f9b", "64cb30ca45885ab48035fbdc47973d6c", "9628dfb9bf005983b1f101a8687f29ab", "730bf651ae6945489c1928a6ddbe0d3d"]



case_dict = {
    "322-744263-23": "b04183e55b355b628fa3d90671422f9b",
    "322-744538-23": "64cb30ca45885ab48035fbdc47973d6c",
    "233-765358-25": "b91f14f71df440179c53cb9e609b0a95", 
    "02-24-00149-CV": "9628dfb9bf005983b1f101a8687f29ab", 
    "24-0395": "a961905f5a2e57d29f2d5436932fdbfe", 
    "02-25-00164-CV": "730bf651ae6945489c1928a6ddbe0d3d", 
    "02-25-00171-CV": "3a939a9942f34058bfc36a1d75c51e83", 
    "02-25-00166-CV": "85636dceaa4d45a4945830ada51632fe", 
    "25-0361": "aac67c62ff5e4f21af99abb9041454ec", 
    "25-0367": "6055079876534410bc13921fce4c2677", 
    "25-0378": "baa3ae6d61c247cb96111cb4182a3893", 
    "25-0426": "8a8aaaeba7744a25b343bf83a142923f"
}

all_dfs = []

for case_num, case_id in case_dict.items():
    print(f"[~] Fetching events for case {case_num}")
    df = fetch_all_case_events(case_id)
    df["case_number"] = case_num  # Add readable label
    all_dfs.append(df)

# Final master DataFrame
master_df = pd.concat(all_dfs, ignore_index=True)
master_df.to_csv("all_case_events.csv", index=False)

print(f"[✓] Combined {len(master_df)} total records across {len(all_dfs)} cases.")
