...
from fudstop.apis.webull.webull_trading import WebullTrading as AsyncWebullSDK
...
sema = Semaphore(10)
db = DbManager(host='localhost', port=5432, user='chuck', password='fud', database='markets')
wb_opts = WebullOptions(access_token=os.environ.get('ACCESS_TOKEN'), did=os.environ.get('DID'), osv=os.environ.get('OSV'), database='markets', user='chuck')
...
async def get_bars(ticker, interval:str='m1', timeStamp=None, conn=None):
    if ticker == 'I:SPX':
        ticker = 'SPXW'
    elif ticker =='I:NDX':
        ticker = 'NDX'
